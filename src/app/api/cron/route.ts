...
const RSI_OVERBOUGHT_THRESHOLD = 55;
const DEEP_RSI_THRESHOLD = 35;
const DEEP_RSI_OVERBOUGHT = 65;
...
export async function GET() {
  ...
  try {
    ...
    let latestCache: { [key: string]: number | null } | null = null; // Store the cache for the most recent candle
    ...
    for (let i = 0; i < Math.min(3, chartData.length); i++) {
      ...
      logCond('Uptrend (price > EMA30)', coreBuyC6);
      const coreBuyTrueCount = [coreBuyC1, coreBuyC2, coreBuyC3, coreBuyC4, coreBuyC5, coreBuyC6, volumeOK, volumeOK1].filter(Boolean).length;
      log(`Core Buy Conditions Met: ${coreBuyTrueCount}/8`);
      ...
      logCond('Downtrend (price < EMA30)', coreSellC6);
      const coreSellTrueCount = [coreSellC1, coreSellC2, coreSellC3, coreSellC4, coreSellC5, coreSellC6, volumeOK, volumeOK1].filter(Boolean).length;
      log(`Core Sell Conditions Met: ${coreSellTrueCount}/8`);

      if (coreBuyC1 && coreBuyC2 && coreBuyC3 && coreBuyC4 && coreBuyC5 && coreBuyC6 && volumeOK && volumeOK1) {
        log('→ Candidate: HIGH BUY (Core Trend-Following)');
        candidates.push({ type: 'BUY', level: 'High', price: latest.close, time: latest.time });
      }
      if (coreSellC1 && coreSellC2 && coreSellC3 && coreSellC4 && coreSellC5 && coreSellC6 && volumeOK && volumeOK1) {
      ...
      const shiftBuyTrueCount = [shiftBuyC1, shiftBuyC2, shiftBuyC3, shiftBuyC4, shiftBuyC5].filter(Boolean).length;
      log(`Shift Buy Conditions Met: ${shiftBuyTrueCount}/5`);
      ...
      const shiftSellTrueCount = [shiftSellC1, shiftSellC2, shiftSellC3, shiftSellC4, shiftSellC5].filter(Boolean).length;
      log(`Shift Sell Conditions Met: ${shiftSellTrueCount}/5`);
      ...
      // Store cache if this is the latest candle
      if (i === 0) {
        latestCache = cache;
      }
    }
    ...
    // Use the stored cache for the most recent candle for final checks
    const latest = chartData.at(-1)!;
    const prev = chartData.at(-2) ?? latest;
    if (!latestCache) {
      log('No latest cache available.');
      return NextResponse.json({ message: 'No latest cache available.' });
    }

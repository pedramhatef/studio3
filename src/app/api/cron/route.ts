...
export async function GET() {
  ...
  try {
    ...
    if (chartData.length < Math.max(EMA_SLOW_PERIOD, BBANDS_PERIOD, RSI_PERIOD, ATR_PERIOD)) {
    ...
    const allIndicatorsAvailable = [latestEmaFast, latestEmaSlow, latestDeepLowerBB, latestEmaMedium,
      latestPSar, latestVwap, latestRsi, previousRsi, latestLowerBB, latestUpperBB, latestAtr].every(v => v !== null && v !== undefined);
